...
chatServer.on("join", function(data){
	document.getElementById("messages").innerHTML += "<p><strong>Server: <i>" + data.username[1] + "</strong> has joined the chat!" + "</i></p>";
});

chatServer.on("leave",function(data){
  document.getElementById("messages").innerHTML += "<p><strong>Server: <i>" + data.username + "</strong> has left the chat." + "</i></p>";  
});



function instantiateTemplate(node, data) {
  // msgDisplay in http://codepen.io/anon/pen/ALaEbg 
  // your code to instantiate template goes here
  return instantiateNode(node, data);
}
...
function instantiateNode(node, data){
  ...
  if (clonedNode.nodeType == document.TEXT_NODE){
    ...
    var arrayOfMatches = clonedNode.nodeValue.match(/{{.+}}/g);//{{2}[^}]+\}{2}  {{2}[^}]+\}{2}
    ...
    if(arrayOfMatches != null){
      for(var j = 0; j < arrayOfMatches.length; j++){
        ...
        if(matchSubstring == "username") {
          ...
          // console.log("data[matchSubstring]: " + data[matchSubstring]);
